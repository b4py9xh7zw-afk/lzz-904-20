...
- **报告导出**: 一键导出 TXT / HTML 调参报告，包含素材信息、滤镜参数、预览缩略图与完整命令，方便剪辑同事复现效果（报告仅内嵌压缩缩略图，不含视频文件）。
...
4. **开始调试**:
   ...
   - 点击 **“生成预览”** 查看右侧结果。
   - 在控制台查看生成的 FFmpeg 命令。
   - 点击 **“TXT 报告 / HTML 报告”** 导出调参报告，发给剪辑同事即可按报告中的参数与命令复现效果。

## 📂 项目结构

- `public/index.php`: 主界面与前端逻辑。
- `public/process.php`: 后端 FFmpeg 调用逻辑与预设保存。
- `public/report.php`: 调参报告（TXT / HTML）生成与导出。
